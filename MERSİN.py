...
for index, row in df.iterrows():
     sql_sorgusu = "EXEC [dbo].[spmersinhalfiyatı] ?, ?, ?, ?, ?, ?, ?, ?, ?"
     sql_sorgusu = "INSERT INTO mersinhalfiyatı (Şube, Ürün, Cinsi, Türü ,MinFiyat, MaxFiyat, OrtFiyat, Birim, Tarih) VALUES (?, ?, ?, ?,?,?,?,?,?)"
     veri = (row['ŞUBE'], row['ÜRÜN'], row['CİNSİ'], row['TÜRÜ'],
             row['Min. Fiyat'].replace(' TL', '').replace(',', '.'),
             row['Mak. Fiyat'].replace(' TL', '').replace(',', '.'),
             row['Ort. Fiyat'].replace(' TL', '').replace(',', '.'),
             row['Birim'], row['Tarih'])
     print(index, row)
     Cursor.commit()
